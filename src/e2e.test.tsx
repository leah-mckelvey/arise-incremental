...
const cleanDatabase = async () => {
  await db.delete(transactions).where(eq(transactions.userId, E2E_TEST_USER_ID));
...
describe('End-to-End Tests', () => {
  ...
  describe('Stress Tests - Button Spamming & Race Conditions', () => {
    ...
    it('should handle concurrent gather requests without losing any', async () => {
      ...
      const promises = Array.from({ length: 10 }, () => gameApi.gatherResource('essence'));
      ...
      const maxEssence = Math.max(...results.map((r) => r.state.resources.essence));
    ...
    it('should handle mixed concurrent operations', async () => {
      ...
      const buildingResponses = results.filter((r) => r.state.buildings?.essenceExtractor);
      const maxBuildings = Math.max(
        ...buildingResponses.map((r) => r.state.buildings.essenceExtractor.count)
      );
    ...
    it('should handle stat allocation spam with limited points', async () => {
      ...
      const promises = Array.from({ length: 5 }, () => gameApi.allocateStat('strength'));
      const results = await Promise.allSettled(promises);
    ...
    it('should maintain consistency under heavy load', async () => {
      ...
      const gatherPromises = Array.from({ length: 25 }, () => gameApi.gatherResource('essence'));
      ...
      const buildingSuccesses = buildingResults.filter((r) => r.status === 'fulfilled');
  ...
  describe('Dungeon System', () => {
    ...
    describe('Named Hunter Requirement', () => {
      it('should require at least one named hunter (Sung Jinwoo) to start a dungeon', () => {
        ...
        dungeonsStore.startDungeon(unlockedDungeon!.id, Date.now(), [SUNG_JINWOO_ID], onSuccess);

        expect(successCalled).toBe(true);
        expect(useDungeonsStore.getState().activeDungeons.length).toBe(1);
        expect(useDungeonsStore.getState().activeDungeons[0].partyIds).toContain(SUNG_JINWOO_ID);
      });
      ...
      it('should allow a named companion to lead when Sung Jinwoo is busy', () => {
        ...
        dungeonsStore.startDungeon(unlockedDungeon!.id, Date.now(), [SUNG_JINWOO_ID], () => {
          firstSuccess = true;
        });
        ...
        const namedAlly = alliesStore.recruitAlly('Cha Hae-In', 'demon-castle-entrance');
      ...
      it('should NOT allow a dungeon to start with only nameless grunts', () => {
        ...
        const hasValidLeader = partyWithOnlyGrunts.includes(SUNG_JINWOO_ID) || isNamedCompanion;
    ...
    describe('Hunter Busy States', () => {
      it('should mark hunters as busy when dungeon is running', () => {
        ...
        const isSungJinwooBusy = activeDungeons.some((ad) => ad.partyIds?.includes(SUNG_JINWOO_ID));
      ...
      it('should prevent busy hunters from joining another dungeon', () => {
        ...
        useDungeonsStore
          .getState()
          .startDungeon(unlockedDungeon!.id, Date.now(), [SUNG_JINWOO_ID], () => {
      ...
      it('should mark hunters as available when dungeon completes', () => {
        ...
        dungeonsStore.startDungeon(unlockedDungeon!.id, startTime, [SUNG_JINWOO_ID], () => {});
    ...
    describe('Party Size Limits', () => {
      ...
      it('should allow parties up to max size', () => {
        ...
        const dungeonsStore = useDungeonsStore.getState();
        const dungeons = dungeonsStore.dungeons;
        const allianceDungeon = dungeons.find((d) => d.unlocked && d.type === 'alliance');
        ...
        if (allianceDungeon) {
          ...
          const soloDungeon = dungeons.find((d) => d.unlocked && d.type === 'solo');
          expect(soloDungeon).toBeDefined();

          let success = false;
          dungeonsStore.startDungeon(soloDungeon!.id, Date.now(), [SUNG_JINWOO_ID], () => {
            success = true;
          });
    ...
    describe('Nameless Grunts Leadership', () => {
      ...
      it('should validate party has valid leader (Sung Jinwoo or named companion)', () => {
        ...
        const hasValidLeader1 =
          ...
          party1.some((id) => {
            const ally = useAlliesStore.getState().allies.find((a) => a.id === id);
