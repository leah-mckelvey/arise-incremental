...
export const ArtifactsTab = () => {
  ...
  const blacksmithXpToNextLevel = useStore(
    useArtifactsStore,
    (state) => state.blacksmithXpToNextLevel
  );
    ...
    <Box>
      ...
      >
        ...
        <Box mt={2} p={2} bg="var(--bg-primary)" rounded="4px">
          ...
          {(Object.entries(calculateTierDropRates(blacksmithLevel)) as Array<[string, number]>).map(
            ([tier, rate]) => (
              <Text
                key={tier}
                style={{
                  color: getTierColor(
                    tier as 'Common' | 'Uncommon' | 'Rare' | 'Epic' | 'Legendary'
                  ),
                  fontSize: '11px',
                }}
              >
                ...
                <Box>
                  ...
                  {artifact ? (
                    <>
                      <Text
                        style={{
                          color: getTierColor(artifact.tier),
                          fontSize: '14px',
                          fontWeight: 'bold',
                        }}
                      >
                      ...
                      {artifact.upgrades.length > 0 && (
                        <Text style={{ color: 'var(--text-dim)', fontSize: '11px' }}>
                          Base: {formatStatBonus(artifact.baseStats)} | +{artifact.upgrades.length}{' '}
                          upgrades
                        </Text>
                      )}
                    </>
                ...
                {artifact && (
                  <Box style={{ display: 'flex', gap: '8px' }}>
                    <Button
                      ...
                      style={{
                        background:
                          artifact.upgrades.length < artifact.maxUpgrades
                            ? 'var(--accent-gold)'
                            : 'var(--bg-tertiary)',
                        border: '1px solid var(--border-color)',
                        color:
      ...
      {inventory.length > 0 && (
        ...
        >
            ...
            {inventory.map((artifact) => (
              ...
              >
                <Box style={{ flex: 1 }}>
                  ...
                  {artifact.upgrades.length > 0 && (
                    <Text style={{ color: 'var(--text-dim)', fontSize: '11px' }}>
                      Base: {formatStatBonus(artifact.baseStats)} | Upgrades:{' '}
                      {artifact.upgrades.map((u) => u.name).join(', ')}
                    </Text>
                  )}
                </Box>
                <Box style={{ display: 'flex', gap: '8px' }}>
          ...
          {inventory.length > 0 && (
            ...
            >
              <Text
                style={{ color: 'var(--text-primary)', fontWeight: 'bold', marginBottom: '10px' }}
              >
                💥 Bulk Destroy
              </Text>
                ...
                {(['E', 'D', 'C', 'B', 'A'] as ArtifactRank[]).map((rank) => {
                  ...
                  return (
                    <Button
                      ...
                      onClick={() => {
                        if (
                          window.confirm(`Destroy all artifacts ≤${rank}-Rank? (${count} items)`)
                        ) {
      ...
      >
          ...
          <Box style={{ display: 'flex', gap: '8px' }}>
            {([1, 5, 10, 100] as const).map((amount) => (
              <Button
                ...
                style={{
                  background:
                    craftBulkAmount === amount ? 'var(--accent-gold)' : 'var(--bg-tertiary)',
        ...
        {ranks.map((rank) => {
          ...
          const lockReason = !canCraftHunter
            ? '(Hunter rank too low)'
            : !canCraftBlacksmith
              ? '(Blacksmith level too low)'
              : '';
          return (
            <Box key={rank} mb={3}>
              <Heading
                level={4}
                style={{ color: canCraft ? 'var(--accent-gold)' : 'var(--text-secondary)' }}
              >
                ...
                {slots.map(({ slot, name, icon }) => {
                  ...
                  return (
                    <Button
                      ...
                      style={{
                        background:
                          canCraft && canAfford ? 'var(--accent-gold)' : 'var(--bg-primary)',
                        border: '1px solid var(--border-color)',
                ...
                >
                  <Box>
                    ...
                    <Text style={{ color: 'var(--text-secondary)' }}>
                      {freshArtifact.tier} {freshArtifact.rank}-Rank {freshArtifact.slot} |
                      Upgrades: {freshArtifact.upgrades.length}/{freshArtifact.maxUpgrades}
                    </Text>
                  </Box>
                  <Box style={{ display: 'flex', gap: '8px' }}>
                    {([1, 5, 10, 100] as const).map((amount) => (
                      ...
                      >
                        ...
                        >
                          ...
                          <Button
                            ...
                            style={{
                              background:
                                canAfford && !isFull ? 'var(--accent-gold)' : 'var(--bg-tertiary)',
                              border: '1px solid var(--border-color)',
