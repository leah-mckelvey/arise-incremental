...
describe("researchStore", () => {
  ...
  describe("Initial State", () => {
    ...
    it("should have correct prerequisites", () => {
      ...
      expect(research.shadowEconomy.requires).toEqual([
        "industrialScale",
        "compoundedLearning",
      ]);
    });
  ...
  describe("purchaseResearch", () => {
    it("should research tech when knowledge is sufficient and no prerequisites", () => {
      ...
      expect(
        useResearchStore.getState().research.basicExtraction.researched,
      ).toBe(true);
    });
    ...
    it("should not research when prerequisites are not met", () => {
      ...
      expect(callbackCalled).toBe(false);
      expect(
        useResearchStore.getState().research.manaResonance.researched,
      ).toBe(false);
    });
    ...
    it("should not research already researched tech", () => {
      ...
      useResearchStore
        .getState()
        .purchaseResearch("basicExtraction", 100, () => {
          callbackCalled = true;
        });
    ...
    it("should handle multiple prerequisites", () => {
      ...
      useResearchStore.setState({
        research: {
          ...
          manaResonance: {
            ...useResearchStore.getState().research.manaResonance,
            researched: true,
          },
          industrialScale: {
            ...useResearchStore.getState().research.industrialScale,
            researched: true,
          },
          knowledgeGeneration: {
            ...useResearchStore.getState().research.knowledgeGeneration,
            researched: true,
          },
          compoundedLearning: {
            ...useResearchStore.getState().research.compoundedLearning,
            researched: true,
          },
        },
      });
  ...
  describe("Research Effects", () => {
    it("should have building efficiency bonuses", () => {
      const basicExtraction =
        useResearchStore.getState().research.basicExtraction;
      expect(
        basicExtraction.effects?.buildingEfficiency?.essenceExtractor,
      ).toBe(1.5);
    });
