...
describe('Game API Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    // Ensure no cross-test pollution from the in-memory query cache
  ...
  describe('0. GET /api/game/state', () => {
    it('0.1 should migrate legacy state to include soulHarvester + shadowEconomy unlocks', async () => {
      // Simulate a legacy DB row created before the server knew about Soul Harvester + unlock metadata
      const legacyBuildings = { ...initialBuildings };
      delete (legacyBuildings as Record<string, unknown>).soulHarvester;

      const legacyResearch = {
        ...initialResearch,
        shadowEconomy: { ...initialResearch.shadowEconomy },
      };
      delete (legacyResearch.shadowEconomy as Record<string, unknown>).unlocks;
      ...
      // We modified the DB directly, so we must invalidate the cache.
      await queryClient.invalidateQueries(['gameState', TEST_USER_ID]);
      ...
      expect(response.body.state.buildings?.soulHarvester).toBeDefined();
      expect(response.body.state.buildings.soulHarvester.id).toBe('soulHarvester');
      expect(response.body.state.research?.shadowEconomy?.unlocks).toEqual(['soulHarvester']);

      const updated = await db.query.gameStates.findFirst({
        where: eq(gameStates.userId, TEST_USER_ID),
      });
      expect(updated).toBeDefined();

      const updatedBuildings = updated!.buildings as Record<string, unknown>;
      expect(updatedBuildings.soulHarvester).toBeDefined();

      const updatedResearch = updated!.research as {
        shadowEconomy?: { unlocks?: unknown };
      };
      expect(updatedResearch.shadowEconomy?.unlocks).toEqual(['soulHarvester']);
    });
  });
  ...
  describe('1. POST /api/game/gather-resource', () => {
    ...
    it('1.4 should cap resources at resourceCaps', async () => {
      ...
      await db.update(gameStates).set({ essence: 990 }).where(eq(gameStates.userId, TEST_USER_ID));
    ...
    it('1.5 should handle duplicate transaction ID', async () => {
      ...
      expect(response1.body.state.resources.essence).toBe(response2.body.state.resources.essence);
    });
  ...
  describe('2. POST /api/game/purchase-building', () => {
    it('2.1 should successfully purchase building with sufficient resources', async () => {
      ...
      expect(response.body.state.buildings.essenceVault.count).toBeGreaterThan(0);
    });

    it('2.2 should fail purchase with insufficient resources (single resource)', async () => {
      // Set essence to low value
      await db.update(gameStates).set({ essence: 10 }).where(eq(gameStates.userId, TEST_USER_ID));
  ...
  describe('3. POST /api/game/purchase-bulk-building', () => {
    it('3.1 should successfully purchase multiple buildings', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.state.buildings.library.count).toBeGreaterThanOrEqual(3);
    });
  ...
  describe('4. POST /api/game/purchase-research', () => {
    it('4.1 should successfully purchase research', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.state.research.basicEfficiency.researched).toBe(true);
    ...
    it('4.2 should fail research with insufficient knowledge', async () => {
      await db.update(gameStates).set({ knowledge: 10 }).where(eq(gameStates.userId, TEST_USER_ID));
  ...
  describe('6. POST /api/game/extract-shadow', () => {
    ...
    it('6.3 should reject invalid dungeon ID', async () => {
      await db.update(gameStates).set({ souls: 2000 }).where(eq(gameStates.userId, TEST_USER_ID));
  ...
  describe('7. Race Conditions & Concurrency', () => {
    it('7.1 should handle concurrent purchases on same resource', async () => {
      await db.update(gameStates).set({ essence: 100 }).where(eq(gameStates.userId, TEST_USER_ID));
    ...
    it('7.3 should handle duplicate clientTxId in concurrent requests', async () => {
      ...
      const [response1, response2] = await Promise.all([
        request(app).post('/api/game/gather-resource').send({ resource: 'essence', clientTxId }),
        request(app).post('/api/game/gather-resource').send({ resource: 'essence', clientTxId }),
      ]);
